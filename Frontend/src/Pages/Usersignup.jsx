  ...
  const submithandler = async (e) => {
    ...
    try {
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        navigate("/user-verification", { state: { email: email } });
      }
    } catch (error) {
    ...
    <div className="p-7 bg-yellow-100 h-screen flex flex-col justify-between">
      ...
      >
        ...
        <button
          type="submit"
          disabled={isLoading}
          className={`bg-yellow-300 rounded-md px-2 py-3 mt-4 w-full text-lg font-normal ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
